...
export default function StockCard({ stock, onClick, onDelete }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[24px] bg-[#FF3B30] mb-6">
      {/* 底部刪除按鈕 */}
      <button
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2 text-[#8E8E93] text-xs font-black">
            <span>{stock.shares} 股</span>
            {/* <span>{stock.updateDate || "無日期"}</span> */}
          </div>
        </div>
        ...
        <div className="flex flex-col items-end">
          ...
          <div
            className={`flex items-center justify-center px-2 py-1.5 rounded-4xl min-w-[10px] font-bold text-[14px] ${
              isPos ? "bg-[#FF3B30]/10 text-[#FF3B30]" : "bg-[#34C759]/10 text-[#34C759]"
            }`}
          >
            <Triangle
              size={10}
